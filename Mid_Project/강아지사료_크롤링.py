...
for pi in range(1,paging_index+1):
    ...
    for i in range(1,paging_size+1):
        ...
        try:
            score_xpath = f'//*[@id="__next"]/div/div[2]/div[2]/div[3]/div[1]/ul/div/div[5]/li/div[1]/div[2]/div[5]/a/span/span'
...
data_df = pd.DataFrame(zip(name_list,link_list,price_list,cat_list,desc_list,review_list,score_list,buy_list,enrollment_list,zzim_list),columns=['이름','링크','가격','분류','설명(전체)','리뷰수','평점','구매건수','등록일','찜하기'])
...
# 엑셀 저장
data_df.to_excel('data_df.xlsx',index=False,encoding = 'utf-8-sig')